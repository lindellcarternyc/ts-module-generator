...
const getParameter = async (): Promise<Parameter> => {
  ...
  return inquirer.prompt<{ parameterName: string }>([getParameterNameQuestion]).then(async ({ parameterName }) => {
...
const getParameters = async (parameters: Parameter[] = []): Promise<Parameter[]> => {
  ...
  return inquirer.prompt<{ addParameter: string }>([AddParamQuestion]).then(({ addParameter }) => {
    if (addParameter) {
...
export default async (): Promise<Parameter[]> => {
  ...
  return inquirer.prompt<{ addParam: boolean }>([GetParamsQuestion]).then(({ addParam }) => {
    if (addParam) {
